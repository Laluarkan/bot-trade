...
class GoldTradingEnv(gym.Env):
    ...
    def __init__(self, features: np.ndarray, prices: np.ndarray, spreads: np.ndarray, dates=None,
        ...
        # --- Sinyal tren: SEKARANG diambil dari data_loader.build_feature_matrix
        # (extra dict) supaya identik antara training & live, bukan dihitung ulang
        # dari raw prices di sini (dulu jadi sumber distribution-shift). Kalau
        # extra tidak diberikan (fallback), hitung sendiri seperti sebelumnya.
        if extra is not None and 'trend_ema' in extra and 'macro_slope' in extra:
            self.trend_ema = np.asarray(extra['trend_ema'], dtype=np.float64)
            self.macro_slope = np.asarray(extra['macro_slope'], dtype=np.float64)
        else:
            self.trend_ema = pd.Series(self.prices).ewm(span=config.TREND_EMA_SPAN, adjust=False).mean().values
            macro_ema = pd.Series(self.prices).ewm(span=config.MACRO_EMA_SPAN, adjust=False).mean().values
            self.macro_slope = (pd.Series(macro_ema).diff(10) / pd.Series(macro_ema)).fillna(0).values * 100.0

        if extra is not None and 'atr' in extra:
            # ATR dalam unit harga (dollar), dipakai basis SL/TP dinamis
            atr_dollars = np.asarray(extra['atr'], dtype=np.float64)
            self.volatility = np.nan_to_num(atr_dollars, nan=0.2)
    ...
    def step(self, action):
        c = self.reward_coeffs  # alpha, beta, gamma, delta
        ...
        if self.dates is not None:
            ...
            is_toxic_hour = config.ENABLE_TOXIC_HOUR_FILTER and (current_time.hour in config.TOXIC_HOURS)
        ...
        # --- Filter tren: sekarang bisa soft (penalti reward) atau hard (blokir aksi)
        # tergantung config.TREND_FILTER_HARD_BLOCK. Default soft -> agen tetap
        # boleh melawan tren kalau memang dia yakin, tapi kena penalti reward.
        if config.ENABLE_TREND_FILTER:
            current_trend_ema = self.trend_ema[self.t]
            current_slope = self.macro_slope[self.t]
            against_trend = False
            if target_position > 0 and (price < current_trend_ema or current_slope <= 0.001):
                against_trend = True
            elif target_position < 0 and (price > current_trend_ema or current_slope >= -0.001):
                against_trend = True

            if against_trend:
                step_reward -= config.TREND_FILTER_PENALTY
                if config.TREND_FILTER_HARD_BLOCK:
                    target_position = 0.0
        ...
        if target_position != self.position:
            if self.position != 0.0 and target_position != 0.0:
                step_reward -= config.CHURN_PENALTY * c['gamma']
            ...
            if self.position != 0.0 and self.entry_price is not None:
                ...
                if pnl > 0:
                    step_reward += (pnl / self.balance) * config.REWARD_SCALE * c['alpha']
                else:
                    step_reward -= (abs(pnl) / self.balance) * config.REWARD_SCALE * c['alpha']
        ...
        step_reward += portfolio_return * config.REWARD_SCALE * c['alpha']

        if target_position != self.prev_position:
            step_reward -= (transaction_cost / self.balance) * config.REWARD_SCALE * c['alpha']
        else:
            if self.position != 0.0:
                current_pnl = self._unrealized_pnl_dollar(price)
                if current_pnl > 0:
        ...
        # --- Penalti drawdown per-step (risk-adjusted reward, dulu tidak ada
        # sama sekali kecuali saat margin call). Mendorong agen menjaga
        # equity curve tetap halus, bukan cuma total profit akhir. ---
        drawdown = max(0.0, (self.peak_value - self.portfolio_value) / max(self.peak_value, 1e-8))
        step_reward -= drawdown * config.REWARD_SCALE * c['beta'] * 0.01

        # --- Penalti idle kecil: kalau posisi flat terus tanpa alasan filter,
        # beri sedikit dorongan supaya agen tidak "membeku" selamanya. Efeknya
        # kecil (delta) supaya tidak memicu overtrading. ---
        if self.position == 0.0 and target_position == 0.0 and self.cooldown_timer == 0:
            self.idle_steps += 1
